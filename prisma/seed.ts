 ...
 *
 * Contenu : 1 ville (Ngaoundéré) + ses zones, et — seulement si ADMIN_EMAIL est
 * renseigné — un AdminUser supplémentaire. Le compte fondateur est intégré au
 * code (server/lib/founder.ts) et créé à sa première connexion : aucune variable
 * n'est nécessaire pour lui. Le catalogue est semé par `npm run prisma:seed:catalog`.
 */
import 'dotenv/config';
...
async function seedAdminUser() {
  if (!process.env.ADMIN_EMAIL?.trim()) {
    console.log('[seed] AdminUser : ADMIN_EMAIL absent — aucun admin supplémentaire (le fondateur est intégré au code).');
    return;
  }
...
async function main() {
  if (process.env.ADMIN_EMAIL?.trim()) adminSeedCredentials(); // Valider avant toute écriture.
