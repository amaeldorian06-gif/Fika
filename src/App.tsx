...
import { captureLeadContext } from './lib/lead';
...
export default function App() {
  // Provenance mémorisée dès la première page (campagne, référent, appareil) — sans cookie.
  useEffect(() => { captureLeadContext(); }, []);
  const { path } = useRoute();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-brand-bg text-brand-text font-sans">
      ...
      {!isAdmin && (
        <aside aria-label="Contact rapide" className="md:hidden fixed bottom-5 right-4 z-40">
          <a
            href={getWALink(
              "Bonjour Fika 👋,\n\nJ'ai un problème à résoudre chez moi. Pouvez-vous m'aider ?"
            )}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-[#25D366] text-white shadow-2xl font-bold text-sm hover:scale-105 active:scale-95 transition-all"
            aria-label="Contacter Fika sur WhatsApp"
          >
