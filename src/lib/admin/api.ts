...
export interface AdminOrderSummary {
  ...
  /** Prestation principale (type de service) — lisible dans la liste et les exports. */
  serviceName: string | null;
  /** Marge brute = prix client − Σ coûts tracés (FCFA) ; null tant que le prix n'est pas fixé. */
...
export interface AdminTask {
  ...
  expertName: string | null;
  expertTrade?: string | null;
  /** Signaux publics du professionnel (même définition que src/lib/trust.ts). */
  expertPublicSignals?: string[];
...
export interface AdminOrderDetail extends AdminOrderSummary {
  ...
  quotes: { id: string; quoteNumber: string; amount: number; status: string; details?: string | null; validUntil?: string | null }[];
...
export interface ExpertMetricsDto {
...
export interface AdminExpert {
  ...
  status: string;
  /** Interventions Fika terminées (calculé depuis les tâches, jamais saisi). */
  ...
  /** Niveau effectif (manuel ou activité, le plus élevé ; « actif » révocable). */
  verificationLevel: string;
  storedVerificationLevel: string;
  lastVerifiedAt: string | null;
  lastVerificationType: string | null;
  firstAssignedAt: string | null;
  metrics: ExpertMetricsDto;
  /** Signaux que le client pourrait voir — définition unique src/lib/trust.ts. */
  publicSignals: string[];
  history: ExpertVerificationDto[];
}
...
/** Lien de téléchargement CSV (même session cookie, ouvert dans le navigateur). */
export function exportUrl(dataset: 'leads' | 'customers' | 'orders' | 'events', from?: string, to?: string): string {
  const params = new URLSearchParams();
  if (from) params.set('from', from);
  if (to) params.set('to', to);
  const qs = params.toString();
  return `/api/admin/export/${dataset}${qs ? `?${qs}` : ''}`;
}
...
/** Devis construit à partir des coûts (coût pro + déplacement + matériel + marge = prix client). */
export async function buildQuote(payload: QuoteBuilderPayload): Promise<{ ok: true; quoteNumber: string }> {
  return await postJson<{ ok: true; quoteNumber: string }>('/api/admin/orders/quotes/build', payload);
}
...
export async function verifyExpert(payload: { expertId: string; to: string; type: string; note?: string }): Promise<{ ok: true; level: string }> {
  return await postJson<{ ok: true; level: string }>('/api/admin/experts/verify', payload);
}
