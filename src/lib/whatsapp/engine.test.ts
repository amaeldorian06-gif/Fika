...
describe('buildOrderIntro', () => {
  ...
  it('intervention / projet : demande de devis, aucun prix ni formule entre parenthèses', () => {
    const plomberie = fakeService({ priceType: 'QUOTE', typeBesoin: 'Intervention', startingPrice: undefined });
    const intro = buildOrderIntro(plomberie, null);
    expect(intro).toContain('Je souhaite un devis pour : Service test');
    expect(intro).not.toMatch(/\(|F\b|Devis selon/);
  });
});

describe('compileQuoteMessage', () => {
  it('le client voit le prix final et ce qu\u2019il couvre, jamais la décomposition ni la marge', () => {
    const msg = flat(compileQuoteMessage({
      orderNumber: 'CMD-2026-0007', quoteNumber: 'DEV-2026-0003', serviceName: 'Dépannage Plomberie',
      clientPrice: 10000, travelIncluded: true, clientNotes: 'Remplacement du flexible compris.', validUntil: '25/09/2026', city: 'Ngaoundéré',
    }));
    expect(msg).toContain('Prix total : 10 000 F');
    expect(msg).toContain('Déplacement / livraison et matériel prévus : compris dans ce prix');
    expect(msg).toContain('Précisions : Remplacement du flexible compris.');
    expect(msg).toContain('valable jusqu\'au 25/09/2026');
    expect(msg).toContain('DEV-2026-0003');
    expect(msg).not.toMatch(/marge|coût professionnel|Décomposition interne/i);
  });
});
...
describe('compileMessage', () => {
  ...
  it('DIAGNOSTIC sans template → gabarit appareil/problème/urgence', () => {
    ...
    expect(msg).toContain("J'ai un problème à faire diagnostiquer : Service test.");
    expect(msg).toContain('le devis (professionnel, déplacement et pièces compris)');
    expect(msg).not.toContain('3 000 F'); // le prix réel est établi par le devis, pas annoncé dans le message
  ...
  it('QUOTE sans template → gabarit devis (projet/ville/délai)', () => {
    ...
    expect(msg).toContain('la marche à suivre pour le devis');
    expect(msg).not.toMatch(/budget estimé|\d\s?F\b/); // aucun montant : le prix vient du devis
  });
  ...
  it('service réel du catalogue (presence-digitale-pro) : message complet et borné', () => {
    const svc = getServiceRef('s-num-3');
    expect(svc).toBeDefined();
    const msg = compileMessage(svc!, { context: 'service-page' });
    expect(msg.length).toBeGreaterThan(80);
    expect(msg.length).toBeLessThan(900); // tient sur mobile
    expect(msg).toContain("Nom de l'activité");
...
describe('compileOrderConfirmation (conformité)', () => {
  it('récapitule commande, prix (déplacement inclus), délai + CGV', () => {
    ...
    expect(msg).toContain('inclus dans le prix ci-dessus (Ngaoundéré)');
    expect(msg).not.toMatch(/gratuit/i);
    expect(msg).toContain('/#/cgv');
