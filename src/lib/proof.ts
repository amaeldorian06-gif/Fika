...
export interface DeliveryQuote {
  /** Part facturée au client en sus du devis : toujours 0 (le déplacement est déjà dans le prix client). */
  customerFee: number;
...
export function quoteDelivery(internalCost: number, deliveryIncluded = true): DeliveryQuote {
  return {
    ...
    label: deliveryIncluded
      ? 'Déplacement / livraison organisés par Fika, intégrés au prix client (Ngaoundéré)'
