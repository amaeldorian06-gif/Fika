...
const CATEGORY_ALIASES: Record<string, string> = {
  ...
  reparation: 'reparation-maintenance',
  transport: 'transport-logistique',
  numerique: 'numerique-documents',
};

/** Tous les univers (catégories) actifs, triés dans l'ordre de présentation. */
export function getCategories(options: { activeOnly?: boolean } = {}): Category[] {
  const { activeOnly = true } = options;
  return UNIVERSES.filter((u) => !activeOnly || u.active !== false);
}

export function getCategoryBySlug(slug: string): Category | undefined {
  const resolvedSlug = CATEGORY_ALIASES[slug] || slug;
  return UNIVERSES.find((u) => u.slug === resolvedSlug);
}

export function getServices(options: { categoryId?: string; activeOnly?: boolean } = {}): Service[] {
  const { categoryId, activeOnly = true } = options;
  const targetCategory = categoryId ? (CATEGORY_ALIASES[categoryId] || categoryId) : undefined;
  return SERVICES
    .filter((s) => (!activeOnly || s.active) && (!targetCategory || s.categoryId === targetCategory))
    .sort(byDisplayOrder);
}
...
export function getPackages(options: { categoryId?: string; activeOnly?: boolean } = {}): Package[] {
  ...
  return UNIVERSE_PACKAGES.filter(
    (p) => (!activeOnly || p.active) && (!targetCategory || p.categoryId === targetCategory),
  );
}
