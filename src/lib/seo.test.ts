...
describe('métadonnées — couverture et unicité', () => {
  ...
  it('le catalogue génère 25+ routes publiques', () => {
    expect(getAllPublicRoutes().length).toBeGreaterThanOrEqual(25);
  });

  it('fiche service : seoTitle/seoDescription priment, JSON-LD Service + Offer', () => {
    const service = getServiceBySlug('intervention-plomberie');
    expect(service).toBeDefined();
    const meta = seoForRoute('/service/intervention-plomberie');
    expect(meta.title).toBe('Dépannage Plomberie à Ngaoundéré | Fika');
    ...
    // Intervention : aucun prix inventé dans le JSON-LD, seulement la description « Devis selon le besoin ».
    expect(offer?.offers?.price).toBeUndefined();
    const cv = seoForRoute('/service/creation-cv-professionnel').jsonLd.find((ld) => ld['@type'] === 'Service') as { offers?: { price?: number } };
    expect(cv?.offers?.price).toBe(3000);
  });
  ...
  it('fiche avec FAQ embarque un bloc FAQPage', () => {
    ...
    const metaSans = seoForRoute('/service/intervention-electricite');
