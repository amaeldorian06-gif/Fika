...
export function ExpertForm({ expert, onSaved, onCancel }: { expert: AdminExpert | null; onSaved: () => void; onCancel: () => void }) {
  ...
  const [trade, setTrade] = useState(expert?.trade ?? '');
  const [references, setReferences] = useState(expert?.references ?? '');
  ...
  return <form className="mb-6 bg-brand-surface border border-brand-border rounded-2xl p-6" onSubmit={async e => {
    ...
    setError(null);
    const parsed = createExpertSchema.safeParse({ name, phone, skills: [...new Set(skills.split(',').map(s => s.trim()).filter(Boolean))], zone: zone.trim() || null, usualCost: cost === '' ? null : Number(cost), availability: available, status, trade: trade.trim() || null, references: references.trim() || null, documents: documents.trim() || null, internalNotes: internalNotes.trim() || null });
    ...
    <fieldset disabled={pending} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <Field label="Métier principal" htmlFor="expert-trade" hint="Tel que déclaré par le professionnel."><Input id="expert-trade" maxLength={80} value={trade} onChange={e => setTrade(e.target.value)} placeholder="Plombier, électricien, maçon…" /></Field>
      <Field label="Compétences" htmlFor="expert-skills" hint="Séparées par des virgules, 20 maximum."><Input id="expert-skills" value={skills} maxLength={1220} onChange={e => setSkills(e.target.value)} placeholder="Fuite, chauffe-eau, installation sanitaire" /></Field>
      ...
      <Field label="Références déclarées" htmlFor="expert-references" hint="Clients ou chantiers précédents cités par le pro (non contrôlés tant qu'ils ne sont pas rappelés)." className="sm:col-span-2"><Textarea id="expert-references" rows={2} maxLength={4000} value={references} onChange={e => setReferences(e.target.value)} /></Field>
      <Field label="Documents fournis" htmlFor="expert-documents" hint="Libellés ou liens (CNI vue le…, attestation, photos de réalisations). Jamais publics." className="sm:col-span-2"><Textarea id="expert-documents" rows={2} maxLength={4000} value={documents} onChange={e => setDocuments(e.target.value)} /></Field>
      <Field label="Notes internes" htmlFor="expert-notes" hint="Visibles uniquement par l'équipe Fika." className="sm:col-span-2"><Textarea id="expert-notes" rows={3} maxLength={4000} value={internalNotes} onChange={e => setInternalNotes(e.target.value)} /></Field>
