...
2. Se connecter avec le **compte fondateur** intégré au code
   (`server/lib/founder.ts` : e-mail + hash bcrypt, jamais le mot de passe en
   clair). Il est créé dans `AdminUser` à la première connexion ; pour changer
   son mot de passe, définir `FOUNDER_PASSWORD_HASH` (hash bcrypt) dans Vercel.
3. D'autres administrateurs (`SUPERADMIN` ou `OPS`) peuvent être ajoutés avec
   `ADMIN_EMAIL` + `ADMIN_PASSWORD_HASH` puis `npm run admin:seed`.
