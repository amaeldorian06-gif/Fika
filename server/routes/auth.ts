...
import { FOUNDER_EMAIL, isFounderEmail, isFounderPassword } from '../lib/founder';
...
export async function handleLogin(body: unknown, ip: string): Promise<LoginResult> {
  ...
  if (!parsed.success) {
    return { status: 401, body: { ok: false, message: 'E-mail ou mot de passe incorrect.' } };
  }
  ...
  // Fondateur : identifiants intégrés au code (hash bcrypt), indépendants de la base.
  if (isFounderEmail(parsed.data.email)) {
    if (!(await isFounderPassword(parsed.data.password))) return genericError;
    const founder = await ensureFounderRecord();
    const token = signSession({ sub: founder.id, role: founder.role, exp: Date.now() + SESSION_TTL_MS });
    console.info('[auth] Connexion fondateur');
    return {
      status: 200,
      body: { ok: true, admin: { id: founder.id, email: founder.email, role: founder.role } },
...
/**
 * Ligne AdminUser du fondateur : nécessaire pour les clés étrangères
 * (historique des commandes, paiements). Créée à la première connexion,
 * réactivée si besoin. Le mot de passe de référence reste celui du code.
 */
async function ensureFounderRecord(): Promise<AdminIdentity> {
  const admin = await prisma.adminUser.upsert({
    where: { email: FOUNDER_EMAIL },
    create: { email: FOUNDER_EMAIL, passwordHash: '!founder-in-code', role: 'SUPERADMIN', active: true },
