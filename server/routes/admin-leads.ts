...
/** Libellé court d'une prestation sur mesure : la ligne « Problème : … » du besoin structuré, sinon le début du texte. */
function customItemName(description: string | null | undefined): string {
  const text = (description ?? '').trim();
  if (!text) return 'Demande sur mesure';
  const problem = text.split('\n').map((l) => l.trim()).find((l) => /^(Problème|Besoin)\s*:/i.test(l));
  const base = (problem ? problem.replace(/^(Problème|Besoin)\s*:\s*/i, '') : text.split('\n').find((l) => l.trim() && !l.trim().startsWith('[')) ?? text).trim();
  return base.length > 120 ? `${base.slice(0, 117)}…` : base || 'Demande sur mesure';
}
    ...
    return await serialTransaction<Result>(async tx => {
      ...
      const order = await tx.order.create({ data: {
        ...
        items: lead.serviceId
          ...
          : { create: [{ customName: customItemName(lead.description), quantity: 1, price: null }] },
