import { createExpertSchema, updateExpertSchema, verifyExpertSchema } from '../../src/lib/admin/experts';
import { canSetLevel, type VerificationLevel, type VerificationType } from '../../src/lib/trust';
import { applyLevelChange } from '../lib/expert-trust';
...
/**
 * Changement du niveau de vérification par un admin, avec trace obligatoire
 * (ExpertVerification). Les niveaux « Intervention Fika réalisée » et
 * « Professionnel actif » sont refusés ici : ils viennent des missions terminées.
 */
export async function handleVerifyExpert(body: unknown, adminId: string | null): Promise<MutationResult<{ level: string }>> {
  if (!adminId) return { status: 401, body: { ok: false, code: 'UNAUTHORIZED', message: 'Session expirée. Reconnectez-vous.' } };
  const parsed = verifyExpertSchema.safeParse(body);
  if (!parsed.success) return { status: 400, body: { ok: false, code: 'VALIDATION', message: parsed.error.issues[0].message } };
  const { expertId, to, type, note } = parsed.data;
  return await serialTransaction<MutationResult<{ level: string }>>(async tx => {
    const expert = await tx.expert.findUnique({ where: { id: expertId }, select: { id: true, verificationLevel: true } });
    if (!expert) return { status: 404, body: { ok: false, code: 'NOT_FOUND', message: 'Professionnel introuvable.' } };
    const check = canSetLevel(expert.verificationLevel as VerificationLevel, to as VerificationLevel, type as VerificationType, note);
    if (!check.ok) return { status: 409, body: { ok: false, code: 'LEVEL_REFUSED', message: check.reason ?? 'Changement refusé.' } };
    await applyLevelChange(tx, expert, to as VerificationLevel, type as VerificationType, note?.trim() || null, adminId);
    return { status: 200, body: { ok: true, level: to } };
  });
}
