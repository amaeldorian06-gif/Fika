...
import { syncAutomaticLevel } from '../lib/expert-trust';
...
export async function handleAssignExpert(
  ...
  return serialTransaction<MutationResult<{ taskId: string; margin: MarginView | null }>>(async tx => {
    ...
    // Un expert engagé n'est plus proposé tant que la mission court ; on date sa première mission Fika.
    await tx.expert.update({ where: { id: expert.id }, data: { availability: false, ...(expert.firstAssignedAt ? {} : { firstAssignedAt: new Date() }) } });
...
export async function handleUpdateTask(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // L'événement sert aussi de trace de performance (début, reprise, fin) — voir server/lib/expert-trust.ts.
    await tx.orderEvent.create({
      data: {
    ...
    // Niveaux de confiance automatiques (Intervention Fika réalisée / Professionnel actif), tracés.
    if (done) {
      for (const a of task.assignments) await syncAutomaticLevel(tx, a.expertId, adminId);
    }
  });
...
export async function handleUpsertDelivery(
  ...
  // Le déplacement est déjà dans le prix client (devis) : on ne trace ici que le coût interne.
  ...
  if (quote.internalCost > alreadyTraced) {
    await prisma.cost.create({
      data: {
        ...
        note: 'Transport / déplacement (intégré au prix client)',
      },
    });
  }
