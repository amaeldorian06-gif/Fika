...
export async function handleUpdateStatus(
  ...
  const order = await prisma.order.findUnique({
    ...
    include: {
      ...
      tasks: { include: { assignments: { select: { id: true } } } },
    },
  });
  if (!order) {
  ...
  const check = canTransition(from, to, {
    ...
    hasConfirmedPayment: order.payments.some((p) => p.status === 'CONFIRMED'),
  ...
  // Statut + historique dans une seule transaction.
  await prisma.$transaction([
    ...
    prisma.orderEvent.create({
      data: { orderId: order.id, from, to, note: parsed.data.note ?? null, actorId: adminId },
    }),
  ]);
...
export async function getOrderMargin(orderId: string): Promise<MarginView | null> {
  const order = await prisma.order.findUnique({
    where: { id: orderId },
...
/** Numéro de devis séquentiel par année : DEV-2026-0001. */
export async function nextQuoteNumber(): Promise<string> {
  const year = new Date().getFullYear();
  const count = await prisma.quote.count({ where: { quoteNumber: { startsWith: `DEV-${year}-` } } });
...
export async function handleCreateQuote(
  ...
  await prisma.$transaction([
    ...
    // Le devis fixe le prix client de référence.
    prisma.order.update({ where: { id: order.id }, data: { totalPrice: parsed.data.amount } }),
    prisma.orderEvent.create({
      data: {
        orderId: order.id,
        from: order.status,
        to: order.status,
        note: `Devis ${quoteNumber} envoyé.`,
        actorId: adminId,
      },
    }),
  ]);
