...
- Variable **obligatoire** : `DATABASE_URL` (chaîne Supabase *Transaction pooler*, port 6543).
  Facultatives : `DIRECT_URL` (migrations), `AUTH_SECRET` (sinon dérivé de `DATABASE_URL`),
  `VITE_WA_PHONE` au build, `TRUST_PROXY_HOPS=1` derrière Vercel, `PORT`.
- Alerte e-mail à chaque demande (optionnel) : `GMAIL_USER`, `GMAIL_APP_PASSWORD` (mot de passe
  d'application Google), `NOTIFY_EMAIL`, `APP_URL`. Sans ces variables, les demandes sont simplement
  enregistrées en base (consultables dans `/admin` → Demandes).
- Export des données : `/admin` → Demandes / Clients / Analytiques → « Télécharger le CSV »
  (`GET /api/admin/export/{leads|customers|orders|events}?from=AAAA-MM-JJ&to=AAAA-MM-JJ`).
- Connexion admin : le **compte fondateur** est intégré au code (`server/lib/founder.ts`,
  e-mail + hash bcrypt — jamais le mot de passe en clair). `ADMIN_EMAIL` / `ADMIN_PASSWORD_HASH`
  ne servent qu'à ajouter d'autres administrateurs (`npm run admin:seed`).
...
Domaine public actuel : `fika00.vercel.app` (défini dans `src/lib/site.ts`, `index.html`,
`public/sitemap.xml`, `public/robots.txt`). Le jour où `fika.cm` est acheté : remplacer dans ces
4 fichiers, ajouter le domaine dans Vercel → Settings → Domains, et mettre `APP_URL` à jour.

Vercel compile la fonction `api/index.ts` avec `api/tsconfig.json` (CommonJS). Ne pas remettre
`"type": "module"` dans `package.json` : cela casse toutes les routes `/api/*` en production
(`FUNCTION_INVOCATION_FAILED`, imports relatifs sans extension).
...
Adresse : **`https://fika00.vercel.app/#/admin`**, également via « Espace équipe ».
Identifiant : le compte fondateur intégré au code (`server/lib/founder.ts`).
Pour changer son mot de passe : définir `FOUNDER_PASSWORD_HASH` (hash bcrypt)
dans Vercel puis redéployer. Aucun identifiant de secours ni mot de passe par défaut.

Pour ajouter un autre compte : configurer `ADMIN_EMAIL` et `ADMIN_PASSWORD_HASH`,
puis `NODE_ENV=production npm run admin:seed`.
